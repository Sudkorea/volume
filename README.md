...
브라우저 보안상 운영체제의 마스터 볼륨이 아니라 이 페이지에 표시되는
YouTube 플레이어의 볼륨을 조절합니다. 플레이어는 조회수 음량을 그대로
`0`부터 `100` 사이의 YouTube 음량으로 적용합니다.

YouTube 영상은 임베드 재생이 허용되어 있어야 합니다. 현재 사용하는
`XsStb0xbF9Q`는 실브라우저에서 임베드 재생과 조회수 음량 반영을 확인했습니다.
